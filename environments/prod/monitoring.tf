...
resource "aws_cloudwatch_metric_alarm" "operations_cpu_high" {
  count = var.create_operations_instance ? 1 : 0

  alarm_name          = "${local.name_prefix}-operations-cpu-high"
  alarm_description   = "Triggers when the production operations instance CPU usage is high."
  ...
  tags = merge(local.common_tags, {
    Name = "${local.name_prefix}-operations-cpu-high"
  })
}
